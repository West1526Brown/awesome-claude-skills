...
- [PWA Android Builder](./pwa-android-builder/) - Turns a website into an installable, offline-capable Progressive Web App on Android — generating the web app manifest, service worker, icons, and the Trusted Web Activity (TWA) path to publish on the Google Play Store.
...
- [Debate Final Gate](./debate-final-gate/) - Final pre-commit verification checklist for a finished SOUL2X competitive-debate speech, catching dropped clash, self-contradiction, overclaim, weak-version rebuttals, and missing weighing right before delivery.
