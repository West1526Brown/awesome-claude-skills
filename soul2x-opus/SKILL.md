...
- **Proximity to burden beats volume (Cat. I).** A side that won more exchanges but off the resolution's actual standard does not win Burden — count what met the burden, not what merely sounded relevant. Volume of points is irrelevant; proximity is everything.
- **Strongest-version engagement is a prerequisite, not a bonus (Cat. II gateway).** A side that answered only a weakened version of an argument is capped in Clash regardless of how clean the rebuttal — shadow-refutation (answering an easier claim while the real one stands) lands in the bottom bands, not the top.
